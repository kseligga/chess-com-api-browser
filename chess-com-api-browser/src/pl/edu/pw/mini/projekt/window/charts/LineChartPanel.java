...
public class LineChartPanel extends JPanel {
	...
	public LineChartPanel(List<Double> scores) {
		this.scores = scores;
	}
	...
	protected void paintComponent(Graphics g) {
		...
		double yScale;
		if (isConstant(scores)) {
			yScale = ((double) getHeight() - 2 * padding - labelPadding) / 2;
		} else {
	...
	public static boolean isConstant(List<Double> myList) {
		Double min = myList.get(0);
		Double max = myList.get(0);
		for (int i = 1; i < myList.size(); i++) {
			Double el = myList.get(i);
			if (el < min) {
				min = el;
			}
			if (el > max) {
